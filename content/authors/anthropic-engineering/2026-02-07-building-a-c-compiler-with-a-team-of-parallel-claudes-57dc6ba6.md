...
- Language Models in Software Development
key_quotes:
- text: With agent teams, multiple Claude instances work in parallel on a shared codebase
    without active human intervention.
  context: Introducing the concept of agent teams working autonomously on software
    development.
- text: Over nearly 2,000 Claude Code sessions and $20,000 in API costs, the agent
    team produced a 100,000-line compiler that can build Linux 6.9 on x86, ARM, and
    RISC-V.
  context: Highlighting the scale and outcome of the experiment with autonomous Claude
    instances.
- text: Write extremely high-quality tests
  context: Emphasizing the importance of high-quality testing in autonomous agent-based
    development.
stance:
  claude_code: positive
  docker: neutral
  git: neutral
evolution_note: This represents an innovative exploration into the capabilities of
  language models for autonomous software development, pushing the boundaries of what's
  achievable.
tags:
- claude
- compiler-development
...
My implementation of parallel Claude is bare-bones. A new bare git repo is created, and for each agent, a Docker container is spun up with the repo mounted to
/upstream
. Each agent clones a local copy to
/workspace
...
Instead, I leave it up to each Claude agent to decide how to act. In most cases, Claude picks up the “next most obvious” problem. When stuck on a bug, Claude will often maintain a running doc of failed approaches and remaining tasks. In the
git repository
...
Time blindness:
Claude can't tell time and, left alone, will happily spend hours running tests instead of making progress. The harness prints incremental progress infrequently (to avoid polluting context) and includes a default
--fast
option that runs a 1% or 10% random sample. This subsample is deterministic per-agent but random across VMs, so Claude still covers all files but each agent can perfectly identify regressions.
...
This project was designed as a capability benchmark. I am interested in stress-testing the limits of what LLMs can just
barely
achieve today in order to help us prepare for what models will reliably achieve in the future.
...
The generated code is not very efficient. Even with all optimizations enabled, it outputs less efficient code than GCC with all optimizations
disabled.
...
*Context: Introducing the concept of agent teams working autonomously on software development.*
...
*Context: Highlighting the scale and outcome of the experiment with autonomous Claude instances.*

> Write extremely high-quality tests

*Context: Emphasizing the importance of high-quality testing in autonomous agent-based development.*

## Related Topics

- [[topics/compiler-development]]
- [[topics/autonomous-agent-teams]]
- [[topics/language-models-in-software-development]]
