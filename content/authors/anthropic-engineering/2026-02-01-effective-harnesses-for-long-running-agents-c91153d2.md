...
key_quotes:
- text: The core challenge of long-running agents is that they must work in discrete
    sessions, and each new session begins with no memory of what came before.
  context: Describing the primary challenge in managing long-running AI agents.
- text: We developed a two-fold solution to enable the Claude Agent SDK to work effectively
    across many context windows.
  context: Introducing their solution to the challenge of long-running AI agents.
- text: This happens even with compaction, which doesn’t always pass perfectly clear
    instructions to the next agent.
  context: Highlighting a failure mode in their approach, where context compaction
    doesn't fully solve continuity issues.
stance:
  Claude Agent SDK: positive
  compaction: neutral
  Opus 4.5: neutral
evolution_note: This article represents a deep dive into solving practical challenges
  in AI development, showcasing a blend of innovation and applied engineering.
tags:
- ai-agents
...
*Context: Highlighting a failure mode in their approach, where context compaction doesn't fully solve continuity issues.*

## Related Topics

- [[topics/ai-development]]
- [[topics/incremental-development]]
- [[topics/task-management-for-ai-agents]]
